...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include beng_nrpe"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/dirict-beng_nrpe/master/files/bootme.ps1')) -ArgumentList ("beng_nrpe")
#</powershell>


  param(
    [string]$role = "beng_nrpe"
  )

  $puppet_source = "https://github.com/relybv/dirict-beng_nrpe.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\beng_nrpe" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\beng_nrpe" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\beng_nrpe"
  }
